...
test("defaults to gpt-realtime-2.1 when nothing is saved", async () => {
  ...
  assert.deepEqual(await store.get(), { activeSipAgent: "gpt-realtime-2.1", selectedAgent: "gpt-realtime-2.1" });
});
...
test("selecting an OpenAI agent arms it; selecting vapi leaves the armed agent alone", async () => {
  const store = createSettingsStore({ dir: await freshDir() });
  assert.deepEqual(await store.select("gpt-live-1"), { activeSipAgent: "gpt-live-1", selectedAgent: "gpt-live-1" });
  assert.deepEqual(await store.select("vapi"), { activeSipAgent: "gpt-live-1", selectedAgent: "vapi" });
